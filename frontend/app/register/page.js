...
import { REGISTER } from '@/lib/constants/testIds/auth';
...
export default function RegisterPage() {
  ...
  async function handleSubmit(e) {
    ...
    if (!phone.trim() || phone.trim().length < 6) return setError(tr('errPhone'));
    if (!password || password.length < 6) return setError(tr('errPassword'));
    if (password !== confirmPassword) return setError(tr('errPasswordMatch'));
    if (!termsAccepted) return setError(tr('errTerms'));
    ...
    <div className="flex min-h-screen flex-col items-center justify-center bg-secondary px-4 py-10">
      ...
      <div className="w-full max-w-sm">
        <div className="mb-6 flex flex-col items-center gap-2 text-center">
          ...
          <p className="text-base text-muted-foreground">{tr('registerSub')}</p>
        </div>

        <div className="rounded-3xl bg-card p-6 shadow-lg">
          <h2 className="text-xl font-bold text-foreground">{tr('registerTitle')}</h2>
          ...
          <form onSubmit={handleSubmit} className="mt-5 space-y-4">
            ...
            <div className="space-y-1.5">
              ...
              <input
                ...
                data-testid={REGISTER.phoneInput}
                className="w-full rounded-xl border-2 border-border bg-white px-4 py-3 text-lg outline-none focus:border-primary"
              />
            </div>
            ...
            <label className="flex items-start gap-3 pt-1">
              <input
                ...
                data-testid={REGISTER.termsCheckbox}
              ...
              <span className="text-sm leading-snug text-muted-foreground">
                ...
                <button
                  ...
                  data-testid={REGISTER.termsLink}
                >
            ...
            >
              ...
              {loading ? tr('registerLoading') : tr('registerButton')}
            </button>
          </form>

          <p className="mt-5 text-center text-base text-muted-foreground">
            {tr('haveAccount')}{' '}
            <Link href="/login" className="font-bold text-primary underline" data-testid={REGISTER.loginLink}>
              {tr('goToLogin')}
            </Link>
          </p>
        </div>
      ...
      {showTerms && (
        <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/40 px-4" onClick={() => setShowTerms(false)}>
          <div
            className="max-h-[80vh] w-full max-w-lg overflow-y-auto rounded-2xl bg-white p-6 shadow-xl"
            onClick={(e) => e.stopPropagation()}
          >
            <div className="mb-3 flex items-center justify-between">
              <h3 className="text-lg font-bold text-foreground">{tr('termsAndConditions')}</h3>
              <button type="button" onClick={() => setShowTerms(false)} className="text-2xl leading-none text-muted-foreground">×</button>
            </div>
            <TermsContent />
          </div>
        </div>
